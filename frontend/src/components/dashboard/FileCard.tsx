...
export const FileCard = memo(function FileCard({
  ...
  return (
    ...
    >
      <div
        draggable={!isFolder}
        onDragStart={(e) => { if (onDragStart) onDragStart(file.id); e.dataTransfer.setData('application/x-telegram-file-id', file.id.toString()); e.dataTransfer.effectAllowed = 'move'; }}
        ...
        className={cn(
          ...
          'bg-card',
          isSelected ? 'border-primary/60 bg-primary/[0.04] shadow-[0_0_0_1px_var(--color-primary)]' : 'border-hairline hover:border-hairline-strong',
      ...
      >
        {thumbnail ? (
          <div className="absolute inset-0">
            <img src={thumbnail} alt={file.name} className="h-full w-full object-cover" loading="lazy" decoding="async" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/10 to-transparent" />
          </div>
        ) : (
          <div className="absolute inset-x-0 top-0 bottom-[64px] flex items-center justify-center">
            <div aria-hidden className="pointer-events-none absolute inset-0 opacity-60" style={{ background: 'radial-gradient(60% 60% at 50% 45%, rgba(124,92,255,0.10) 0%, transparent 70%)' }} />
            {isFolder ? (
              ...
              <div className="relative"><FileTypeIcon filename={file.name} size="lg" /></div>
            )}
          </div>
        )}
        ...
        {/* Selection circle */}
        <div
          onClick={(e) => { e.stopPropagation(); if (onToggleSelection) onToggleSelection(); }}
          className={cn(
            ...
            isSelected ? 'bg-primary border-primary scale-100' : 'border-white/30 bg-black/40 opacity-0 backdrop-blur-sm group-hover:opacity-100'
          )}
        >
        ...
        {/* Filename strip */}
        <div className={cn('absolute right-0 bottom-0 left-0 border-t px-3 py-2.5', thumbnail ? 'border-transparent text-white' : 'border-hairline-soft bg-card/60 backdrop-blur-[2px]')}>
          <h3 className={cn('truncate text-[13px] leading-snug font-medium', thumbnail ? 'text-white' : 'text-foreground')} title={file.name}>{file.name}</h3>
          <p className={cn('mt-0.5 text-[11px]', thumbnail ? 'text-white/70' : 'text-slate')}>{file.sizeStr}</p>
        </div>
      </div>
    </div>
  );
