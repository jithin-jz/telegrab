    ...
    try:
        ...
        if latest_tag > current_version:
            ...
            for asset in data.get("assets", []):
                ...
                if is_mac and (".dmg" in name or ".zip" in name):
                    download_url = asset["browser_download_url"]
                    break
                elif not is_mac and ".exe" in name:
                    download_url = asset["browser_download_url"]
                    break
...
def cmd_download_and_install_update(download_url: str) -> None:
    ...
    try:
        req = urllib.request.Request(download_url, headers={"User-Agent": "Telegrab-Updater"})
    ...
    try:
        ...
        raise RuntimeError(f"Failed to launch update: {e}")
